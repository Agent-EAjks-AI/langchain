...
def detect_ssn(content: str) -> list[PIIMatch]:
    """Detect US Social Security Numbers in content.

    Detects both formats: 123-45-6789 and 123456789.

    Args:
        content: Text content to scan.

    Returns:
        List of detected SSN matches.
    """
    matches = []

    # Format with dashes: 123-45-6789
    pattern_dashed = r"\b\d{3}-\d{2}-\d{4}\b"
    for match in re.finditer(pattern_dashed, content):
        matches.append(
            PIIMatch(
                type="ssn",
                value=match.group(),
    ...
    # Format without dashes: 123456789 (must be exactly 9 digits)
    pattern_plain = r"\b\d{9}\b"
    for match in re.finditer(pattern_plain, content):
        # Skip if already matched with dashes
        if not any(
...
def _apply_mask_strategy(content: str, matches: list[PIIMatch]) -> str:
    ...
    for match in sorted_matches:
        ...
        if pii_type == "email":
            ...
            # Show last 4: ***-**-1234
            masked = f"***-**-{value[-4:]}" if "-" in value else f"*****{value[-4:]}"
            ...
            masked = (
                f"**{separator}**{separator}**{separator}**{separator}**{separator}{value[-2:]}"  # noqa: E501
            )
...
class PIIMiddleware(AgentMiddleware):
    ...
    to handle them. It can detect emails, SSNs, credit cards, IP addresses,
    ...
    Strategies:
        ...
        - ``mask``: Partially mask PII (e.g., ``***-**-1234`` for SSN)
    ...
    Example:
        ...
        # Block SSNs and credit cards with different strategies
        agent = create_agent(
            "openai:gpt-5",
            middleware=[
    ...
    def __init__(
        self,
        pii_type: Literal["email", "ssn", "credit_card", "ip", "mac_address", "url"] | str,
        *,
        ...
        Args:
            pii_type: Type of PII to detect. Can be a built-in type
                (``email``, ``ssn``, ``credit_card``, ``ip``, ``mac_address``, ``url``)
    ...
    def before_model(
        ...
        """Check user messages for PII before model invocation.
