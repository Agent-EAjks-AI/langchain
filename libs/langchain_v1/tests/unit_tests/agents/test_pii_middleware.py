...
class TestSSNDetection:
    ...
    def test_detect_ssn_without_dashes(self):
        content = "SSN: 987654321"
        matches = detect_ssn(content)

        assert len(matches) == 1
        assert matches[0]["value"] == "987654321"

    def test_no_ssn(self):
        content = "This has no SSN."
        matches = detect_ssn(content)
        assert len(matches) == 0

    def test_partial_ssn_not_detected(self):
        # Should not match partial patterns
        content = "12-34-567 or 1234567"
        matches = detect_ssn(content)
        assert len(matches) == 0
...
class TestRedactStrategy:
    ...
    def test_redact_ssn(self):
        middleware = PIIMiddleware("ssn", strategy="redact")
        state = {"messages": [HumanMessage("SSN: 123-45-6789")]}
...
class TestMaskStrategy:
    ...
    def test_mask_ssn(self):
        middleware = PIIMiddleware("ssn", strategy="mask")
        state = {"messages": [HumanMessage("SSN: 123-45-6789")]}
...
class TestHashStrategy:
    ...
    def test_hash_is_deterministic(self):
        middleware = PIIMiddleware("ssn", strategy="hash")

        # Same SSN should produce same hash
        state1 = {"messages": [HumanMessage("SSN: 123-45-6789")]}
        state2 = {"messages": [HumanMessage("SSN: 123-45-6789")]}
...
class TestBlockStrategy:
    ...
    def test_block_with_multiple_matches(self):
        middleware = PIIMiddleware("ssn", strategy="block")
        state = {"messages": [HumanMessage("SSNs: 123-45-6789 and 987-65-4321")]}
...
class TestMultipleMiddleware:
    ...
    def test_sequential_application(self):
        ...
        state = {"messages": [HumanMessage("Email: test@example.com, SSN: 123-45-6789")]}
        result1 = email_middleware.before_model(state, None)

        # Then apply SSN middleware to the result
        ssn_middleware = PIIMiddleware("ssn", strategy="mask")
        state_with_email_redacted = {"messages": result1["messages"]}
        result2 = ssn_middleware.before_model(state_with_email_redacted, None)
    ...
    def test_note_about_duplicate_middleware(self):
        """Note: create_agent doesn't allow duplicate middleware class names.

        To handle multiple PII types, either:
        1. Apply middleware sequentially (as shown in test_sequential_application)
        2. Use a custom detector that handles multiple patterns
        3. Subclass PIIMiddleware for each type (not recommended)
        """
        # This would raise AssertionError due to duplicate class names:
        # create_agent(model, middleware=[
        #     PIIMiddleware("email", strategy="redact"),
        #     PIIMiddleware("ssn", strategy="mask"),  # Duplicate PIIMiddleware!
        # ])

        # Instead, use a custom detector for multiple types:
        def detect_email_and_ssn(content):
            from langchain.agents.middleware.pii import detect_email, detect_ssn

            return detect_email(content) + detect_ssn(content)

        middleware = PIIMiddleware(
            "email_or_ssn",
            detector=detect_email_and_ssn,
            strategy="redact",
        )
